...
class Model:
        ...
        if not self.subject_data.is_empty():
            ...
            for b_idx in range(1, self.options.num_rooms + 1):   # iterate through the bandits and do choice trials for each bandit
                ...
                for j in range(1, self.options.room_len + 1):
                    choice_probs[trial_idx], trial_rec = self.do_choice_trial(trial_rec)
                    trial_idx += 1
    ...
    def do_choice_trial(self, trial_rec) -> tuple[int, int]:
        ...
        if self.options.simulate_subject:   # Simulation
            # Generate payoffs
            if self.task_vars["trial_idx"]:
                pass

    def do_mem_probe(self, trial_rec, mem_rec) -> tuple[int, int]:
        pass
